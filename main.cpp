...
// INFER
	...
	int idx = i_batch*batch;
        ...
        int M = network->layers[0]->n;
        int K = network->layers[0]->size * network->layers[0]->size * network->layers[0]->c;
        int N = network->layers[0]->out_w * network->layers[0]->out_h;
        
        // pre-process for layer shape
        int height   = network->layers[0]->h;
        int width    = network->layers[0]->w;
        int channels = network->layers[0]->c;
        int ksize    = network->layers[0]->size;
        int stride   = network->layers[0]->stride;
        int pad      = network->layers[0]->padding;
        
        int height_col   = (height+2*pad-ksize)/stride+1;
        int width_col    = (width+2*pad-ksize)/stride+1;
        int channels_col = channels*ksize*ksize;
        
        float *B0 = (float *)malloc(height_col*width_col*channels_col*sizeof(float));
        int w_offset, h_offset, c_im, row, col, col_index, out_index;
        
        // conv
        for(int i = 0; i < network->layers[0]->batch; i++){
            for (int c = 0; c < channels_col; c++) {
                ...
                for (int h = 0; h < height_col; h++) {
                    for (int w = 0; w < width_col; w++) {
                        ...
                        out_index = i*height*width*channels + c_im*height*width + row*width + col;
                        ...
                        if (row < 0 || col < 0 || row >= height || col >= width) {
                            ...
                            B0[col_index] = network->input[out_index];
                        }
                        //printf("%d, %d\n", out_index, col_index);
                    }
                }
            ...
            for(int j = 0; j < N; j++){
                for(int p = 0; p < M; p++){
                    for(int q = 0; q < K; q++){
                        float a_part = network->layers[0]->weights[p*K+q];
                        network->layers[0]->output[i*M*N+p*N+j] += a_part*B0[q*N+j];
                    //printf("%f, %f, %f\n", B0[q*N+j], a_part, network->layers[0]->output[i*M*N+p*N+j]);
                    }
                }
            }
        }
        // add bias
        // # of images
        for(int b = 0; b < network->layers[0]->batch; b++) {
            // # of feature maps per image / otuput channels
            for(int p = 0; p < M; p++) {
                // # of pixel per feature map
                for(int q = 0; q < N; q++) {
                    network->layers[0]->output[b*M*N+p*N+q] += network->layers[0]->biases[p];
                    //printf("%f\n", network->layers[0]->output[b*N*M+p*N+q]);
                }
            }
        }
        // relu
        for (int i = 0; i < network->layers[0]->batch*M*N; i++){
            if (network->layers[0]->output[i] < 0) network->layers[0]->output[i] = 0.0001*network->layers[0]->output[i];
            //printf("%f\n", network->layers[0]->output[i]);
        }
        ...
        height     = network->layers[1]->h;
        width      = network->layers[1]->w;
        channels   = network->layers[1]->c;
        ksize      = network->layers[1]->size;
        stride     = network->layers[1]->stride;
        pad        = network->layers[1]->padding;
        
        int height_out   = network->layers[1]->out_h;
        int width_out    = network->layers[1]->out_w;
        
        float val;
        int valid;
        int cur_h, cur_w;
        
        for(int b = 0; b < network->layers[1]->batch; b++){
                    ...
                    for(int j = 0; j < width_out; j++){
                        ...
                        for(int n = 0; n < ksize; n++){
                            for(int m = 0; m < ksize; m++){
                                ...
                                val = -FLT_MAX;
                                if (valid != 0) {val = network->layers[0]->output[col_index];}
                                if (val > max) {max = val; max_i = col_index;}
                                //printf("%d, %d\n", out_index, col_index);
                                //printf("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", b, k, i, j, m, n, height, width, channels, cur_h, cur_w);
                            }
                        }
                        network->layers[1]->output[out_index] = max;
                        network->layers[1]->indexes[out_index] = max_i;
                    }
                }
            }
        }
        ...
        M = network->layers[2]->n;
        K = network->layers[2]->size * network->layers[2]->size * network->layers[2]->c;
        N = network->layers[2]->out_w * network->layers[2]->out_h;
        
        // pre-process for layer shape
        height   = network->layers[2]->h;
        width    = network->layers[2]->w;
        channels = network->layers[2]->c;
        ksize    = network->layers[2]->size;
        stride   = network->layers[2]->stride;
        pad      = network->layers[2]->padding;
        
        height_col   = (height+2*pad-ksize)/stride+1;
        width_col    = (width+2*pad-ksize)/stride+1;
        channels_col = channels*ksize*ksize;
        
        float *B2 = (float *)malloc(height_col*width_col*channels_col*sizeof(float));
        
        // conv
        for(int i = 0; i < network->layers[2]->batch; i++){
            for (int c = 0; c < channels_col; c++) {
                w_offset = c%ksize;
                h_offset = (c/ksize)%ksize;
                c_im     = (c/ksize)/ksize;
                for (int h = 0; h < height_col; h++) {
                    for (int w = 0; w < width_col; w++) {
                        ...
                        if (row < 0 || col < 0 || row >= height || col >= width) {
                            ...
                            B2[col_index] = network->layers[1]->output[out_index];
                        }
                        //printf("%d, %d\n", out_index, col_index);
                        //printf("%f\n", network->layers[1]->output[out_index]);
                    }
                }
            }
            ...
            for(int j = 0; j < N; j++){
                for(int p = 0; p < M; p++){
                    for(int q = 0; q < K; q++){
                        float a_part = network->layers[2]->weights[p*K+q];
                        network->layers[2]->output[i*M*N+p*N+j] += a_part*B2[q*N+j];
                        //printf("%f, %f, %f\n", B2[q*N+j], a_part, network->layers[0]->output[i*M*N+p*N+j]);
                    }
                }
            }
        }
        // add bias
        // # of images
        for(int b = 0; b < network->layers[2]->batch; b++) {
            // # of feature maps per image / otuput channels
            for(int p = 0; p < M; p++) {
                // # of pixel per feature map
                for(int q = 0; q < N; q++) {
                    network->layers[2]->output[b*M*N+p*N+q] += network->layers[2]->biases[p];
                    //printf("%f\n", network->layers[0]->output[b*N*M+p*N+q]);
                }
            }
        }
        // relu
        for (int i = 0; i < network->layers[2]->batch*M*N; i++){
            if (network->layers[2]->output[i] < 0) network->layers[2]->output[i] = 0.0001*network->layers[2]->output[i];
            //printf("%f\n", network->layers[0]->output[i]);
        }
        // skip-connection
        float *temp;
        temp = (float *)malloc(network->layers[2]->batch*M*N*sizeof(float));
        for (int i = 0; i < network->layers[2]->batch; i++){
            for (int j = 0; j < M; j++) {
                for (int k = 0; k < N; k++) {
                    temp[i*M*N+j*N+k] = network->layers[0]->output[i*M/4*N*2+j/4*N*2+k*2];
                }
            }
        }
        for (int i = 0; i < network->layers[2]->batch*M*N; i++){
            network->layers[2]->output[i] += temp[i];
        }
        ...
        height     = network->layers[3]->h;
        width      = network->layers[3]->w;
        channels   = network->layers[3]->c;
        ksize      = network->layers[3]->size;
        stride     = network->layers[3]->stride;
        pad        = network->layers[3]->padding;
        
        height_out   = network->layers[3]->out_h;
        width_out    = network->layers[3]->out_w;
        
        for(int b = 0; b < network->layers[3]->batch; b++){
            for(int k = 0; k < channels; k++){
                for(int i = 0; i < height_out; i++){
                    for(int j = 0; j < width_out; j++){
                        out_index = b*height_out*width_out*channels + k*height_out*width_out + i*width_out + j;
                        ...
                        for(int n = 0; n < ksize; n++){
                            for(int m = 0; m < ksize; m++){
                                ...
                                valid = (cur_h >= 0 && cur_h < height && cur_w >= 0 && cur_w < width);
                                val = -FLT_MAX;
                                if (valid != 0) {val = network->layers[2]->output[col_index];}
                                if (val > max) {max = val; max_i = col_index;}
                                //printf("%d, %d\n", out_index, col_index);
                                //printf("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", b, k, i, j, m, n, height, width, channels, cur_h, cur_w);
                                //printf("%f\n", network->layers[2]->output[col_index]);
                            }
                        }
                        network->layers[3]->output[out_index] = max;
                        network->layers[3]->indexes[out_index] = max_i;
                    }
                }
            }
        }
        ...
        K = network->layers[4]->inputs;
        N = network->layers[4]->outputs;
        
        // gemm
        for(int i = 0; i < network->layers[4]->batch; i++){
            for(int j = 0; j < N; j++){
                float sum = 0;
                for(int k = 0; k < K; k++){
                    sum += network->layers[3]->output[i*K+k]*network->layers[4]->weights[j*K+k];
                }
                network->layers[4]->output[i*N+j] += sum;
            }
        }
        // add bias
        for(int b = 0; b < network->layers[4]->batch; b++) {
            for(int p = 0; p < N; p++) {
                network->layers[4]->output[b*N+p] += network->layers[4]->biases[p];
                //printf("%f\n", network->layers[4]->output[b*N+p]);
            }
        }
        // relu
        for (int i = 0; i < network->layers[4]->batch*N; i++){
            if (network->layers[4]->output[i] < 0) network->layers[4]->output[i] = 0.0001*network->layers[4]->output[i];
            //printf("%f\n", network->layers[4]->output[i]);
        }
        ...
        K = network->layers[5]->inputs;
        N = network->layers[5]->outputs;
        
        // gemm
        for(int i = 0; i < network->layers[5]->batch; i++){
            for(int j = 0; j < N; j++){
                float sum = 0;
                for(int k = 0; k < K; k++){
                    sum += network->layers[4]->output[i*K+k]*network->layers[5]->weights[j*K+k];
                }
                network->layers[5]->output[i*N+j] += sum;
            }
        }
        // add bias
        for(int b = 0; b < network->layers[5]->batch; b++) {
            for(int p = 0; p < N; p++) {
                network->layers[5]->output[b*N+p] += network->layers[5]->biases[p];
                //printf("%f\n", network->layers[5]->output[b*N+p]);
            }
        }
        // relu
        for (int i = 0; i < network->layers[5]->batch*N; i++){
            if (network->layers[5]->output[i] < 0) network->layers[5]->output[i] = 0.0001*network->layers[5]->output[i];
            //printf("%f\n", network->layers[5]->output[i]);
        }
        ...
        K = network->layers[6]->inputs;
        N = network->layers[6]->outputs;
        
        // gemm
        for(int i = 0; i < network->layers[6]->batch; i++){
            for(int j = 0; j < N; j++){
                float sum = 0;
                for(int k = 0; k < K; k++){
                    sum += network->layers[5]->output[i*K+k]*network->layers[6]->weights[j*K+k];
                }
                network->layers[6]->output[i*N+j] += sum;
            }
        }
        // add bias
        for(int b = 0; b < network->layers[6]->batch; b++) {
            for(int p = 0; p < N; p++) {
                network->layers[6]->output[b*N+p] += network->layers[6]->biases[p];
                //printf("%f\n", network->layers[6]->output[b*N+p]);
            }
        }
        /*
        // relu
        for (int i = 0; i < network->layers[6]->batch*N; i++){
            if (network->layers[6]->output[i] < 0) network->layers[6]->output[i] = ;
            //printf("%f\n", network->layers[6]->output[i]);
        }
         */
        ...
        //printf("n: %d\n", n);
        // softmax
        N = network->layers[7]->inputs;
        //printf("N: %d\n", N);
        for (int b = 0; b < network->layers[7]->batch; b++) {
            float largest = -FLT_MAX;
            float sum = 0;
            for(int i = 0; i < N; i++){
                if(network->layers[6]->output[b*N+i] > largest) largest = network->layers[6]->output[b*N+i];
            }
            //printf("largest: %f\n", largest);
            for(int i = 0; i < N; i++){
                float e = exp(network->layers[6]->output[b*N+i]-largest);
                //printf("e: %f\n", e);
                //printf("diff: %f\n", largest - network->layers[6]->output[b*N+i]);
                network->layers[7]->output[b*N+i] = e;
                sum += e;
            }
            //printf("sum: %f\n", sum);
            for(int i = 0; i < N; i++) {
                network->layers[7]->output[b*N+i] = network->layers[7]->output[b*N+i]/sum;
            }
        }
...
#pragma omp target data map(tofrom:output)
{
    // conv
